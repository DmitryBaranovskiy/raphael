...
(function () {
    ...
    Set[proto].scaleAsGroup = function( w, h ){
        h=h===undefined?w:h;
        var wh2 = Math.sqrt(w*h);
        for(var i=0, il=this.items.length; i< il; i++){
          ...
          switch( item.type ){
            case 'path':
              ...
              item.attr({
                path                : newPath,
                'stroke-width'      : item.attr( 'stroke-width' ) * wh2,
                'stroke-miterlimit' : item.attr( 'stroke-miterlimit' ) * wh2
              });
              break;
            case 'circle':
              item.attr({
            ...
            case 'rect':
              item.attr({
                x                   : item.attr('x') * w,
                y                   : item.attr('y') * h, 
                width               : item.attr('width') * w,
                height              : item.attr('height') * w,
                r                   : item.attr('r') * wh2,
                'stroke-width'      : item.attr('stroke-width') * wh2,
                'stroke-miterlimit' : item.attr( 'stroke-miterlimit' ) * wh2                
              });
            ...
            case 'image':
              item.attr({
                ...
                y                   : item.attr('y') * h, 
                width               : item.attr('width') * w,
                height              : item.attr('height') * h
              });
              break;
            case 'text':
              for(var k=0, kl=item.transformations.length; k< kl; k++){
                if(item.transformations[k].indexOf('rotate')>-1){
                  var rotation = item.transformations[k];
                };
              }
              if( typeof rotation == 'string' ){
                var vals = rotation.match( /[0-9\-\+\.]+/g );
                item.rotate( 0, parseFloat(vals[1]), parseFloat(vals[2]) );
              }
              item.attr({
                font                : (function(){
              ...
              if( typeof rotation == 'string' ){
                item.rotate( parseFloat(vals[0]), parseFloat(vals[1])*w, parseFloat(vals[2])*h );
              }
              break;  
          }
